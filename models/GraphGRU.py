...
class GraphGRUCell(nn.Module):
    ...
    def __init__(self,
        ...
        if init_edge_weights is None:
            self.init_edge_weights = nn.Parameter(
                torch.Tensor(input_size[0], input_size[0]))
            self.__need_train = True
        else:
            self.init_edge_weights = nn.Parameter(
                init_edge_weights, requires_grad=False)
            self.__need_train = False

        self.graph_gates = nn.Linear(
            input_size[1] + hidden_size, 2 * hidden_size, bias=self.bias)
        self.graph_can = nn.Linear(
            input_size[1] + hidden_size, hidden_size, bias=self.bias)
    ...
    def forward(self, input, hx_prev):
        ...
        # (b, n, d_h)
        weighted_hx_prev = torch.einsum(
            "bid, ij -> bjd", hx_prev, edge_weights.t())
...
class GraphGRU(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers):
            cur_input_size = input_size if i == 0 else (
                input_size[0], hidden_size)
